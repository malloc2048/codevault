...
const char MarketData::TICK_PARAMETER_DELIMITER('|' );

const std::string MarketData::QUERY_STR( "QUERY" );
const std::string MarketData::QUERY_PARAMETER_DELIMITER("\"" );
...
void MarketData::run_query() {
    ...
    for( itr = _queries.begin(); _queries.end() != itr; ++itr ) {
        ...
        if( 0 < params.size() ) {
            ...
            } else if( std::string::npos != itr->find( QUERY_BEST_RETURNS_FOR_TICKER_IN_TIME_RANGE ) && 3 == params.size() ) {
...
void MarketData::make_tick( std::string str, int count ) {
    std::vector<std::string> params = parse_ticker_str(str, TICK_PARAMETER_DELIMITER );
...
std::vector<std::string> MarketData::parse_query_str( std::string& str ) {
    ...
    std::size_t start = str.find(QUERY_PARAMETER_DELIMITER );
    while( std::string::npos != start ) {
        std::size_t stop = str.find(QUERY_PARAMETER_DELIMITER, start + 1 );
        params.push_back( str.substr( start + 1, stop - start - 1 ) );
        start = str.find(QUERY_PARAMETER_DELIMITER, stop + 1 );
    }
    return params;
}

std::vector<std::string> MarketData::parse_ticker_str( std::string& str, const char delimiter ) {
    std::vector<std::string> params;
